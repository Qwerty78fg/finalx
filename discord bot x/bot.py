...
def guilds():
    global user_ids_global  # Use the global list
    user_index = request.args.get('user_index')  # Get the index of the user from the query parameters
...
def get_user_ids():
    ...
    active_user_ids = [user_id for user_id in user_ids_global if user_id in user_access_tokens]
    return jsonify(active_user_ids)  # Return the filtered list of user IDs
...
def admin_panel():
    if not session.get('logged_in'):
        return redirect('/')  # Redirect to the login page if not logged in
    return render_template('index.html')  # Render the admin panel template
...
def ban_user():
    ...
    if user_id in user_access_tokens:
        del user_access_tokens[user_id]  # Remove the user from the access tokens
